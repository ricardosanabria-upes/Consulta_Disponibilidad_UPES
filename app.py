...
def cargar_datos():
    # --- RESERVAS (SHEETS) ---
    try:
        # Leemos ignorando nombres de columnas (skiprows=1) y forzando texto
        r_raw = pd.read_csv(URL_SHEETS, skiprows=1, header=None, dtype=str)
        res = pd.DataFrame()
        # Usamos ÍNDICES NUMÉRICOS estrictos para que no importe el nombre de la columna
        res['u'] = r_raw.iloc[:, 3] # Columna D
        res['f'] = r_raw.iloc[:, 6] # Columna G
        res['a'] = r_raw.iloc[:, 7].str.strip() # Columna H
        res['hi'] = r_raw.iloc[:, 8] # Columna I
        res['hf'] = r_raw.iloc[:, 9] # Columna J
    ...
    try:
        ...
        lista_h = []
        columnas_aula = [c for c in h_raw.columns if c not in ["Dia", "Hora"]]
        for _, fila in h_raw.iterrows():
            try:
                ...
                for a in columnas_aula:
                    v = str(fila[a]).strip()
                    ocu = (v != "" and v.lower() != "nan")
                    lista_h.append({
                        "dia": str(fila["Dia"]), "hora_t": h_txt, "hi": hi, "hf": hf,
                        "aula": a.strip(), "info": v if ocu else "Disponible", 
                        "tipo": "C" if ocu else "L"
                    })
            except: continue
...
if df_h is not None:
    ...
    aula_sel = col1.selectbox("Aula/Instalación", sorted(df_h["aula"].unique()))
    ...
    # Obtener bloques del aula
    bloques = df_h[df_h["aula"] == aula_sel].drop_duplicates("hora_t").sort_values("hi")

    for _, b in bloques.iterrows():
        # 1. ¿Hay Clase? (Exacto)
        clase = df_h[(df_h["aula"] == aula_sel) & (df_h["dia"] == dia_txt) & (df_h["hora_t"] == b["hora_t"]) & (df_h["tipo"] == "C")]
        
        if not clase.empty:
            st.error(f"**{b['hora_t']}** | CLASE: {clase.iloc[0]['info']}")
        else:
            # 2. ¿Hay Reserva? (Validación estricta de tiempo y aula)
            reservado = False
            if not df_r.empty:
                # Comparamos que la reserva sea para ESTA aula (basado en la primera palabra para evitar errores de formato)
                id_aula = aula_sel.split()[0]
                m_res = df_r[(df_r["f_dt"] == fecha_sel) & (df_r["a"].str.contains(id_aula, na=False))]
                
                for _, r in m_res.iterrows():
                    # Si el bloque horario se cruza con la reserva
...
with st.expander("Ver Datos Crudos (Para revisión)"):
    st.write("Reservas encontradas:", len(df_
