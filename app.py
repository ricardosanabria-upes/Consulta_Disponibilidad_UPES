...
@st.cache_data(ttl=5)
def load():
    # 1. RESERVAS: Extraer aulas reales de la columna 7
    try:
        r_raw = pd.read_csv(URL_S, skiprows=1, header=None, dtype=str)
        df_r = pd.DataFrame()
        df_r['u'] = r_raw.iloc[:, 3] # Usuario
        df_r['f'] = pd.to_datetime(r_raw.iloc[:, 6], dayfirst=True, errors='coerce').dt.date
        # Limpiar nombre de aula: solo "A-21", "SUM", etc.
        df_r['a_full'] = r_raw.iloc[:, 7].str.strip()
        df_r['a_id'] = df_r['a_full'].str.split().str[0].str.upper()
    ...
    try:
        ...
        list_h = []
        # Solo tomamos columnas que empiecen con 'A-' o sean 'SUM' o 'AUDITORIO'
        # Esto evita que aparezcan "Escuela de Civil" como instalaciones
        cols = [c for c in h_raw.columns if any(x in str(c).upper() for x in ["A-", "SUM", "AUDI", "CENTRO"])]
        ...
        for _, f in h_raw.iterrows():
            try:
                ...
                for a in cols:
                    ...
                    list_h.append({
                        "d": str(f["Dia"]), "h": t, "hi": hi, "hf": hf, 
                        "a": a.strip(), "id": a.split()[0].upper(), 
                        "info": val if occ else "L", "tipo": "C" if occ else "L"
                    })
            except: continue
...
if hor_data is not None:
    # FILTRO CRÍTICO: Solo mostrar instalaciones válidas
    opciones = sorted(hor_data["a"].unique())
    a_sel = st.selectbox("Seleccione Instalación Real", opciones)
    ...
    for _, b in blqs.iterrows():
        ...
        if not c.empty:
            ...
            # B. RESERVA (Comparación por ID de aula para que coincida A-21 con A-21 C/Acondicionado)
            r_match = res_data[(res_data["f"] == f_sel) & (res_data["a_id"] == a_id)]
...
st.divider()
# Corregido el error de sintaxis del st.write
total = len(res_data)
st.write(f"Conexión estable. Reservas detectadas: {total}")
